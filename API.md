...
Pictographs — emoji, dingbats, the miscellaneous symbol blocks (`💙`, `🐱`, `✨`, `⏳`) — are a third class resolved the same way, against a symbol fallback chain (`Segoe UI Emoji`, `Apple Color Emoji`, `Noto Color Emoji`, then the monochrome symbol faces such as `Segoe UI Symbol`). They need a chain of their own because no text face carries them: the face a document declares for body text is a text face, and the CJK chain is made of text faces too, so an emoji resolved by name lands on a font with no glyph for it and is drawn as a `.notdef` box. A run is split wherever the class changes, so a bullet written `💙 布偶猫（Ragdoll）` draws its emoji from the symbol face and its Chinese from the CJK face, and each substitution is reported in `FontDiagnostics`. Note that this applies to slide text: chart labels are still resolved to a single face per string, so a pictograph in a chart label is not covered by this chain.
...
象形字符——emoji、装饰符号、各类杂项符号区（`💙`、`🐱`、`✨`、`⏳`）——是第三类，按同样方式在符号回退链（`Segoe UI Emoji`、`Apple Color Emoji`、`Noto Color Emoji`，其后是 `Segoe UI Symbol` 等单色符号字体）里挑选。它需要单独的链，是因为没有任何文字字体会带这些字形：文档给正文声明的字体是文字字体，CJK 回退链里也全是文字字体，于是按名字解析出来的 emoji 落到一款没有该字形的字体上，被画成 `.notdef` 方框。渲染器会在字符类别变化处切分 run，因此一条 `💙 布偶猫（Ragdoll）` 的项目符号会用符号 face 画 emoji、用 CJK face 画汉字，每次替换都会记录进 `FontDiagnostics`。注意这适用于幻灯片正文；图表文字仍按整串解析成单个 face，因此图表标签里的象形字符不在该回退链的覆盖范围内。
